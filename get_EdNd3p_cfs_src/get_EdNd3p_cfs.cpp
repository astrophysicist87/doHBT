...
//cerr << "plumberg: (&FOsurf_ptr[0])->particle_mu[particle_id] = " << (&FOsurf_ptr[0])->particle_mu[particle_idx] << endl;
   ...
   output << "Calculating Fourier expansion of single-particle spectra..." << endl;
   Source_function.Get_EdNd3p_cfs(FOsurf_ptr);
   Source_function.Output_ev_EdNd3p_cfs(folderindex);
   output << "Finished calculating Fourier expansion of single-particle spectra" << endl;
